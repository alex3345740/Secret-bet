...
declare_id!("4bJWKYAfAjuQDCHxxn57bXfq44jawFUwc7W16zKYgUXj");

pub const SLOTS_ROUND_SEED: &[u8] = b"slots_round";
pub const GAME_TYPE_SLOTS: u8 = 3;
...
#[program]
pub mod hidden_bet_slots {
    use super::*;

    pub fn create_round_slots(ctx: Context<CreateRoundSlots>, args: CreateRoundSlotsArgs) -> Result<()> {
        ...
        emit!(RoundCreated {
            ...
            player: round.player,
            game_type: GAME_TYPE_SLOTS,
        });

        Ok(())
    }

    pub fn place_private_bet_slots(
        ctx: Context<PlacePrivateBetSlots>,
        args: PlacePrivateBetSlotsArgs,
    ) -> Result<()> {
        let now = Clock::get()?.unix_timestamp;
        let round = &mut ctx.accounts.round;
        require!(
            round.status == STATUS_CREATED,
            HiddenBetSlotsError::InvalidRoundStatus
        );
        require_keys_eq!(
            round.player,
            ctx.accounts.player.key(),
            HiddenBetSlotsError::Unauthorized
        );

        round.bet_commitment = args.bet_commitment;
        round.private_payload_hash = args.private_payload_hash;
        round.reel_commitment = args.reel_commitment;
    ...
    pub fn request_randomness_slots(
        ctx: Context<RequestRandomnessSlots>,
        client_seed: [u8; 32],
    ) -> Result<()> {
        ...
        require_keys_eq!(
            ...
            HiddenBetSlotsError::InvalidOracleQueue
        );
        ...
        let ix = create_request_randomness_ix(RequestRandomnessParams {
            ...
            callback_discriminator: instruction::ConsumeRandomnessCallbackSlots::DISCRIMINATOR
                .to_vec(),
    ...
    pub fn consume_randomness_callback_slots(
        ctx: Context<ConsumeRandomnessCallbackSlots>,
        randomness: [u8; 32],
    ) -> Result<()> {
        let now = Clock::get()?.unix_timestamp;
        let round = &mut ctx.accounts.round;
        ...
        require!(
            ...
            HiddenBetSlotsError::ReplayDetected
        );
    ...
    pub fn settle_round_slots(
        ctx: Context<SettleRoundSlots>,
        args: SettleRoundSlotsArgs,
    ) -> Result<()> {
        let now = Clock::get()?.unix_timestamp;
        let round = &mut ctx.accounts.round;
        require!(
...
pub struct CreateRoundSlotsArgs {
    pub round_id: u64,
    pub config_commitment: [u8; 32],
}

#[derive(AnchorSerialize, AnchorDeserialize, Clone, Copy)]
pub struct PlacePrivateBetSlotsArgs {
    pub bet_commitment: [u8; 32],
    pub private_payload_hash: [u8; 32],
    pub reel_commitment: [u8; 32],
}

#[derive(AnchorSerialize, AnchorDeserialize, Clone, Copy)]
pub struct SettleRoundSlotsArgs {
    pub outcome_commitment: [u8; 32],
    pub settlement_commitment: [u8; 32],
}

#[derive(Accounts)]
#[instruction(args: CreateRoundSlotsArgs)]
pub struct CreateRoundSlots<'info> {
    #[account(mut)]
    pub player: Signer<'info>,
    #[account(
        init,
        payer = player,
        space = 8 + SlotsRound::INIT_SPACE,
        seeds = [SLOTS_ROUND_SEED, player.key().as_ref(), &args.round_id.to_le_bytes()],
        bump
    )]
    pub round: Account<'info, SlotsRound>,
    pub system_program: Program<'info, System>,
}

#[derive(Accounts)]
pub struct PlacePrivateBetSlots<'info> {
    #[account(mut)]
    pub player: Signer<'info>,
    #[account(
        mut,
        seeds = [SLOTS_ROUND_SEED, player.key().as_ref(), &round.round_id.to_le_bytes()],
        bump = round.bump
    )]
    pub round: Account<'info, SlotsRound>,
}
...
#[vrf]
#[derive(Accounts)]
pub struct RequestRandomnessSlots<'info> {
    #[account(mut)]
    pub player: Signer<'info>,
    #[account(
        mut,
...
#[derive(Accounts)]
pub struct ConsumeRandomnessCallbackSlots<'info> {
...
#[derive(Accounts)]
pub struct SettleRoundSlots<'info> {
    #[account(mut)]
    pub player: Signer<'info>,
    #[account(
        mut,
